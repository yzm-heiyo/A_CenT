...
public class TransTagFragment extends BaseFragment implements View.OnClickListener {
...
//        icon8.setOnClickListener(this);
    ...
    private void refreshView(List<PropertyFastSearcherTag> tags) {
        ...
        for (int i = 0; i < tags.size(); i++) {
            L.d("TransItemValue", tags.get(i).getItemValue());
            switch (i) {
                case 0:
        ...
        try {
            ...
            if (itemValue.indexOf(",") != -1) {
                for (String s1 : itemValue.split(",")) {
                    ...
                    request = setParams(request, param[0], param[1]);
                }
            } else {
                String[] param = itemValue.split(":");
                if(param[0].equals("PropertyTypes"))param[0]="RoomCounts";
                request = setParams(request, param[0], param[1]);
            }
            L.d("Tag_HouseDes", request.toString());
            new BaseClass().notifyBusMessage(BUS_MESSAGE.HomeFastSearch.FAST_TRAN_SEARCH, request);
    ...
    public <T> T setParams(T data, String key, String value) throws IllegalAccessException {
        ...
        for (Field field : data.getClass().getDeclaredFields()) {
            ...
            L.d("TransParams", "TransKey:" + key + " FileName: " + field.getName());
            if (field.getName().equals(key)) {
                L.d("TransParams", "equare");
